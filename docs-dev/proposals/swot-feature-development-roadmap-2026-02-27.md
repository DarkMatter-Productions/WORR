...
- `FR-04-T12` / `DV-07-T06` In Progress:
  ...
  - First behavior/action dispatcher units now compile as WORR-native `bot_actions.*`, `bot_items.*`, and `bot_combat.*` boundaries; `bot_brain.*` samples the dispatcher every accepted command frame, applies validated attack/use decisions through `BotActions_ApplyDecisionDetailed()`, records pending weapon/inventory intents, and emits `q3a_bot_action_status`. Follow-up Phase 4/6 support now adds a per-bot perception blackboard, Q2/WORR weapon metadata, item utility scoring hooks, health/armor focused routing, combat enemy-fact/damage-attribution proof helpers, weapon-switch request/observation proof state, deterministic health/armor pickup proof helpers, target-source-aware team-objective proof helpers, validated action-layer command requests for weapon/inventory intents, and deterministic return/support team-role policy helpers. Modes `20` through `23` now use those hooks as implemented smoke scenarios, while deeper aim, firing, autonomous command dispatch, and objective intelligence remains future work.
  - Scenario and performance validation tooling now exists under `tools/bot_scenarios/` and `tools/bot_perf/`, covering nine implemented scenario smokes, a manual high-bot soak/degradation policy row, the now-empty pending scenario gap report, semantic promotion checks for modes `20` through `23`, raw reserved-mode diagnostics, split-marker raw metric merging from frame/blackboard/action/objective/source-counter status lines, reserved-mode runtime warmup waiting, derived perf budgets, guarded comparison reports, fixture-aware parser tests, analyzer support for optional source-counter fields, import/adapter source-counter getters, split source-counter status emission, validated bot-frame/route/Q3A-route/static-BSP/entity-clip CPU timing fields, and Q3A memory source-counter reporting.
  - Botfile profile work now includes Q3/Gladiator-style companion families, idTech3-style `botfiles/scripts/*_s.c` companions, multi-skill character validation, script-package coverage, script parity validation, loose `botfiles` mirroring for no-zlib dedicated builds, and user-facing profile docs. The profile-backed spawn scenario remains implemented, but profile metadata is still mostly a behavior-policy hint.
  - The latest promotion wave connects bot-attributed damage to the real damage path, health/armor pickup observation to successful item touches, and CTF objective pickup/return/capture observations to authoritative gameplay branches. Modes `20` through `23` now pass as implemented smoke scenarios for enemy engagement, weapon switching, health/armor pickup, and team-objective completion.
...
- [ ] `FR-04-T04` Add team mode awareness (CTF/TDM/etc.) to bot utility state updates.
  Dependency: `FR-04-T02`. Priority: P1.
  Progress: Team-objective helper/status vocabulary now exists for enemy flag, own flag return, neutral flag, and base-defense assignments. The current proof-hook wave wires authoritative CTF flag pickup, return, and capture observations from the real gameplay branches into objective counters, and runtime mode `23` now passes as the `team_objective` smoke scenario. Durable route/reach policy and broader role ownership across real team flows remain pending.
  Implementation logs: `docs-dev/q3a-botlib-team-objective-helper-scaffold-2026-06-18.md`, `docs-dev/q3a-botlib-team-objective-proof-2026-06-18.md`, `docs-dev/q3a-botlib-ctf-objective-gameplay-hooks-2026-06-18.md`.
...
- [x] `DV-03-T05` Add bot scenario tests for spawn, navigation, and objective behavior.
  ...
  Progress: `tools/bot_scenarios/run_bot_scenarios.py` now runs implemented bot scenario smokes for spawn-to-item routing, stalled-route recovery, eight-bot reservation pressure, map-change repeat validation, profile-backed spawning from staged botfiles, enemy engagement, weapon switching, health/armor pickup, and team objective proof. It emits text, JSON, declarative catalog output, Markdown reports, lightweight historical comparisons, pending-gap reports, and semantic marker-backed promotion checks. Server smoke modes `20` through `23` now validate through frame-command, blackboard, action, objective, and source-counter markers, and the latest implemented run reports 9 passed, 0 failed, 0 timed out, and 0 pending. The follow-up pending-gap report has 0 rows and `overall=ready`. `tools/bot_scenarios/test_run_bot_scenarios.py` covers parser/report behavior and optionally validates `.tmp/bot_scenarios/latest_report.json` when present.
  Latest promotion update: raw diagnostics preserve marker event order and latest source lines, missing metrics report their expected source marker, the prior blocked mode `22` gate has been superseded by the promoted `health_armor_pickup` row, and the CTF objective hooks now feed the implemented `team_objective` smoke.
...
- [ ] `DV-07-T06` Maintain imported-source credits and provenance ledgers for the Q3A BotLib and `TTimo/bspc` AAS work.
  ...
  Latest credit/status update: the WORR-owned map-change restart lifecycle smoke, natural movement support diagnostics, behavior/action telemetry bridge, action-application helpers, health/armor focus routing, raw reserved-mode diagnostics, team-objective helper scaffold, scenario pending-gap report, perf analyzer guard slices, split source-counter plumbing, route CPU timing notes, first-party botfile/profile/script packaging and script-parity work, source-aware reserved-mode diagnostics, CTF objective hook notes, real proof-hook notes, mode `20` through `23` scenario-promotion/CPU-status validation, static BSP and entity-clip CPU timing, AAS memory source counters, high-bot degradation/budget work, release packaging hardening, reference-map coverage reporting, weapon/inventory command APIs, and team-role policy helpers are recorded with no new upstream imports claimed.
