...
- `FR-04-T12` / `DV-07-T06` In Progress:
  ...
  - The latest promotion waves connect bot-attributed damage to the real damage path, health/armor pickup observation to successful item touches, CTF objective pickup/return/capture observations to authoritative gameplay branches, live-aim firing proof, deterministic item-timer policy proof, trace-backed corner-cutting proof, FFA/TDM/coop readiness proof, duel queue/spectator proof, default-off FFA roam-route ownership, default-off FFA spawn-camp avoidance, default-off team role-route ownership, default-off team item-role pickup scoring, default-off team friendly-fire attack suppression, default-off team role-combat attack ownership, default-off CTF role-route ownership, default-off CTF role-combat ownership, default-off CTF dropped-flag route ownership, default-off CTF carrier-support route ownership, default-off CTF base-return route ownership, default-off CTF objective-route policy ownership, default-off CTF objective-route precedence over generic role routing, default-off coop progress-wait, interaction-retry, LeadAdvance, resource-share, anti-blocking, target-sharing, and door/elevator source-hold proofs, and stricter scenario marker checks for live aim plus match-policy evidence. Modes `20` through `45`, `sv_bot_team_policy_smoke` modes `2` and `3`, plus the coop reuse rows now pass as implemented smoke scenarios.
