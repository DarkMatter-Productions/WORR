...
- Added archived cvar `cg_weapon_bob` with default `2`:
...
June 2026 follow-up: `cg_weapon_bob` is now the primary public cvar name to match WORR's cvar naming rules. The original `cg_weaponBob` spelling remains as a non-archived compatibility alias.
...
### Cvar and ownership
- `src/game/cgame/cg_entity_api.cpp` registers `cg_weapon_bob` as `CVAR_ARCHIVE` with default `2`.
- `src/game/cgame/cg_entity_api.cpp` also registers legacy `cg_weaponBob` as a non-archived alias and syncs changes in either direction.
...
- `cg_weapon_bob 0` and `bobskip 1` return the base view pose and reset bob state so re-enabling a bob mode starts cleanly.
- `cg_weapon_bob 1` uses the interpolated server/playerstate weapon pose, which is generated by `p_view.cpp` with the Quake 3 weapon-angle bob constants:
  - roll: `xyspeed * bobfracsin * 0.005`
  - yaw: `xyspeed * bobfracsin * 0.01`
  - pitch: `xyspeed * bobfracsin * 0.005`
  - gun origin/angle interpolation follows the original active viewweapon path so the cvar changes the visible weapon pose instead of using a parallel cgame-only bob cycle
- `cg_weapon_bob 2` keeps the shared stride/landing/idle pieces and adds Doom 3-inspired:
...
- `src/game/cgame/cg_tent.cpp`
  ...
  - This keeps local beam origins aligned with `cg_weapon_bob` and also honors `bobskip` for cgame beams.

### UI and docs
- `src/game/cgame/ui/worr.json` adds an Effects menu dropdown bound to `cg_weapon_bob`:
  - `off` -> `0`
  - `Quake 3` -> `1`
  - `Doom 3` -> `2`
- `docs-user/client.asciidoc` documents `cg_weapon_bob` and its three values.
...
```powershell
.install\worr_x86_64.exe --bootstrap-skip-update-check +set basedir E:\Repositories\WORR\.install +set r_renderer opengl +set vid_fullscreen 0 +set logfile 1 +set logfile_flush 1 +set logfile_name cg_weapon_bob_startup_smoke +set cg_weapon_bob 2 +wait 60 +quit
```
