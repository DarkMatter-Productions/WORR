...
sv_worr_native_snapshot_shadow::
    Adds the server half of the developer-only canonical snapshot diagnostic
    to `sv_worr_native_shadow`. Default value is 0 (disabled). The client and
    server must enable both base native-shadow controls and both snapshot-shadow
    controls before connecting, and `sv_snapshot_shadow` must remain enabled.
    Established snapshot delivery remains available; this control alone does
    not promote native rendering, effects, audio, or gameplay authority. It may
    be combined with the event-shadow pair for the exact combined diagnostic.
    Reconnect clients after changing it. See
    link:progressive-networking-controls.md[Progressive Networking Controls].
...
g_lag_compensation::
    ...
    server-authoritative historical path for eligible deathmatch weapon traces.
    Clients cannot supply a target, hit result, or arbitrary rewind time. The
    supported historical query policies are Machinegun, Chaingun, Shotgun,
    Super Shotgun, Railgun, Disruptor convergence, Plasma Beam, and
    Thunderbolt. Spectators are excluded. This remains an evaluation feature
    while broader mover, lifecycle, fairness, load, and platform release gates
    are incomplete.

sg_lag_compensation_max_ms::
    Maximum historical player rewind window in milliseconds. Default value is
    200 and the server-enforced hard policy limit is 0 to 250. Interpolation
    allowance and client data cannot raise that cap. Value 0 prevents
    historical rewind even when ‘g_lag_compensation’ is enabled.
...
Historical geometry answers collision queries only. Damage, spawn protection,
health, death, and respawn remain current server authority, and a respawn
cannot reuse the previous life's identity or history. A teleport, map or life
change, identity mismatch, marked discontinuity, incomplete scene, or missing
history fails closed to an uncompensated current-world trace. A rejected
canonical command never falls back to a legacy time estimate. Set
‘g_lag_compensation’ to 0 for the server-wide opt-out; no client setting can
override it. Detailed diagnostics identify the selected path, weapon policy,
fallback reason, timing, candidate count, and authority guard without changing
gameplay.
