...
{
    if (Cmd_Argc() < 3) {
        Com_PrintfLoc("$e_auto_bd8795aad49c", Cmd_Argv(0), Cmd_Argv(1));
        return;
    }
    ...
    if (cls.state == ca_cinematic) {
        Com_PrintfLoc("$e_auto_aa73cd5fd87e");
        return;
    }

    const char *path = lookup_track_path(Cmd_Argv(2));
    if (!path) {
        Com_PrintfLoc("$e_auto_71c2c825dc61", Cmd_Argv(2));
        return;
    }
...
static void OGG_Info_f(void)
{
    if (ogg.fmt_ctx)
        ...
        Com_PrintfLoc("$e_auto_79b47d22b1d3");
}
...
static void OGG_Cmd_f(void)
{
    ...
    if (!strcmp(cmd, "info"))
        ...
        Com_PrintfLoc("$e_auto_9b38c06a5ed1", Cmd_Argv(0));
}
