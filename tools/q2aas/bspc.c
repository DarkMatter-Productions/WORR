...
/*
 * Modified for WORR 2026-06-16:
 * - Run reachability/clustering for Quake II maps through the WORR Q2 trace
 *   adapter instead of limiting BSP-to-AAS reachability to Quake III maps.
 * - Treat failed BSP loads as fatal in conversion paths so invalid input fails
 *   clearly before AAS generation can continue with stale or missing map data.
 */
...
extern	void Q2_FreeMaxBSP(void);
...
static qboolean AAS_ShouldCalculateReachForLoadedMap(void)
{
	return loadedmaptype == MAPTYPE_QUAKE3 || loadedmaptype == MAPTYPE_QUAKE2;
}

static void AAS_FreeRetainedBSPForLoadedMap(void)
{
		...
		switch(comp)
		{
			case COMP_BSP2MAP:
			{
				...
				for (qf = qfiles; qf; qf = qf->next)
				{
					...
					//
					if (!LoadMapFromBSP(qf)) Error("error loading BSP map %s\n", qf->origname);
					//write the map file
			...
			case COMP_BSP2AAS:
			{
				...
				for (qf = qfiles; qf; qf = qf->next)
				{
					...
					if (AAS_ShouldCalculateReachForLoadedMap()) AAS_CalcReachAndClusters(qf);
					//
					...
					//deallocate memory
					AAS_FreeMaxAAS();
					AAS_FreeRetainedBSPForLoadedMap();
				} //end for
				break;
			} //end case
			case COMP_REACH:
			{
				...
				for (qf = qfiles; qf; qf = qf->next)
				{
					...
					//if it's a Quake3 map calculate the reachabilities and clusters
					if (AAS_ShouldCalculateReachForLoadedMap())
					{
