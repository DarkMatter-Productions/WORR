 ...
 * - Route Quake II BSP reachability through a WORR-native Q2 trace adapter.
 */
...
#define WORR_Q2AAS_MAPTYPE_QUAKE2 2
...
{
	if (WorrQ2AAS_Q2TraceActive()) return WorrQ2AAS_EntityString();
...
void BotImport_Trace(bsp_trace_t *bsptrace, vec3_t start, vec3_t mins, vec3_t maxs, vec3_t end, int passent, int contentmask)
{
	...
	if (WorrQ2AAS_Q2TraceActive())
	{
		WorrQ2AAS_Trace(bsptrace, start, mins, maxs, end, passent, contentmask);
		return;
	}
...
{
	if (WorrQ2AAS_Q2TraceActive()) return WorrQ2AAS_PointContents(p);
...
void BotImport_BSPModelMinsMaxsOrigin(int modelnum, vec3_t angles, vec3_t outmins, vec3_t outmaxs, vec3_t origin)
{
	...
	if (WorrQ2AAS_Q2TraceActive())
	{
		WorrQ2AAS_BSPModelMinsMaxsOrigin(modelnum, angles, outmins, outmaxs, origin);
		return;
	}
...
void AAS_CalcReachAndClusters(struct quakefile_s *qf)
{
	...
	if (loadedmaptype == WORR_Q2AAS_MAPTYPE_QUAKE2)
	{
		Log_Print("loading Q2 collision map...\n");
		if (!WorrQ2AAS_BeginQ2Reach(qf, &aasworld.bspchecksum))
		{
			Log_Print("Q2 reachability skipped: WORR Q2 trace adapter could not initialize\n");
			return;
		}
	}
	else
	{
		WorrQ2AAS_EndQ2Reach();
